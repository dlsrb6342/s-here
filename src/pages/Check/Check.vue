	...
	methods: {
		...
		retTimeList: function () {
			...
			xhr.onreadystatechange = function () {
				this.showTimeTable = JSON.parse(xhr.responseText).hasOwnProperty('data')
				if (JSON.parse(xhr.responseText).hasOwnProperty('data')) this.retData = JSON.parse(xhr.responseText).data
				else {
					this.retData = []
					alert('조회에 실패하였습니다.')
				}
