              ...
              <v-dialog v-model="dialog" width="480" persistent>
                <v-card>
                  ...
                  <v-card-text>
                    <v-layout row wrap>
                      <v-flex xs5>
                        <v-select label="시작 시각" :items="hourItems" v-model="selFromTimeHour" item-text="hour"></v-select>
                      </v-flex>
                      <v-flex xs5>
                        <v-select :items="minItems" v-model="selFromTimeMin" item-text="min"></v-select>
                      </v-flex>
                      <v-flex xs2>
                        <v-card-text class="subheading">부터</v-card-text>
                      </v-flex>
                      <v-flex xs5>
                        <v-select label="종료 시각" :items="hourItems" v-model="selToTimeHour" item-text="hour"></v-select>
                      </v-flex>
                      <v-flex xs5>
                        <v-select :items="minItems" v-model="selToTimeMin" item-text="min"></v-select>
                      </v-flex>
                      <v-flex xs2>
                        <v-card-text class="subheading">까지</v-card-text>
                      </v-flex>
                      <v-flex xs8>
                        <v-slider label="사용시간" :max="47-click.first" :min='1' v-model="duration" class="subheading"></v-slider>
                      </v-flex>
                      <v-flex xs4>
                        <v-card-text class="subheading">{{ parseInt(duration/2) == 0 ? '' : parseInt(duration/2)+'시간'}}{{ duration % 2 == 0 ? '00' : '30'}}분</v-card-text>
                      </v-flex>
                      <v-flex xs12>
                        <v-select label="동반이용자" class="subheading" autocomplete :async-loading="loading" chips multiple :items="retPeople" item-text="text" item-value="_id" :search-input.sync="search" v-model="sendData.people" :debounce-search="500" clearable>
                          <template slot="selection" scope="data">
                            <v-chip @input.native="data.parent.selectItem(data.item)" :selected="data.selected" close>
                              <strong>{{ data.item.name }}</strong>
                              <span class="caption">({{ data.item.studentId }})</span>
                            </v-chip>
                          </template>
                        </v-select>
                      </v-flex>
...
export default {
  ...
  watch: {
    search(val) { val && val.length >= 5 && this.querySelections(val) },
    duration(val) {
      this.click.second = this.click.first + val
      this.selToTimeHour = this.hourItems[parseInt((this.click.first + val) / 2)]
      this.selToTimeMin = this.minItems[(this.click.first + val) % 2]
    }
  },
  created() {
          ...
          if (xhr.readyState === XMLHttpRequest.DONE) {
            ...
            if (result.hasOwnProperty('data')) {
              ...
              for (let i in [...Array(self.product.length * 48).keys()]) self.TimeTable.push({
                text: (self.hourItems[parseInt(i / self.product.length / 2)].hour + ' ' + self.minItems[parseInt(i / self.product.length) % 2].min),
                state: 'empty'
              })
              for (let item of result.data) {
    ...
    querySelections: function(val) {
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == XMLHttpRequest.DONE) {
          ...
          for (let item of JSON.parse(xhr.responseText).data) {
            self.retPeople.push({
              text: item.name + '(' + item.email + ', ' + item.studentId + ')',
              name: item.name,
